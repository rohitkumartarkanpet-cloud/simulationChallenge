...
This project explores a coin‑flip investment game played once per year from age 20 to age 75.

- Starting balance at age **20**: **$30,000**  
- Each year until age **75**: flip a fair coin  
  - Heads → multiply the account by **1.5** (50% gain)  
  - Tails → multiply the account by **0.6** (40% loss)
...
- What does a single lifetime path look like from 20 to 75?  
- If many people play this game, what does the distribution of final balances look like?  
- How often do we retire with more than the original $30,000?  
- How does the 25% strategy change both the risk and the upside?
...
Picture the very first year, right after investing the $30,000 at age 20.

Outcomes after that first coin flip:

- Heads: the account jumps to $45,000 (probability 0.5)  
- Tails: the account falls to $18,000 (probability 0.5)
...
So, looked at in isolation, the game promises about a **5% boost** to wealth after one year. The catch is that we keep flipping this coin every year for **55 years**, and compounding can amplify both winning streaks and losing streaks.
...
To get an intuitive feel for the risk, I start with one sample lifetime where the **entire account is exposed every single year**.
...
**Story of this path.**  
The account wanders around for decades, sometimes above the starting level and often below it. A bad cluster of tails late in life can wipe out years of earlier wins. In this particular run, the final balance at age 75 is much lower than $30,000, which already suggests that “bet everything every year” is a dangerous way to fund retirement.

## 100 simulations – distribution for the 100% strategy

One lifetime shows how things *can* go. To see what usually happens, I simulate **100 independent lifetimes** from age 20 to 75 with the 100% strategy and record the final balance at age 75.
...
# Log-spaced bins so bars widen smoothly to the right
...
# Color bars by whether their geometric center is below or above $30k
for i, patch in enumerate(patches):
    center = np.sqrt(bin_edges[i] * bin_edges[i+1])
    if center < target_balance:
        patch.set_facecolor("#ff6b6b")   # below target
        patch.set_hatch("///")
    else:
        patch.set_facecolor("#26c6da")   # at/above target
        patch.set_hatch("..")

# Log-normal guide curve
...
# Reference lines: buy-in, median, mean
...
# Compact stats badge in the corner
badge = (
...
```{python}
#| label: full-percentile-table
#| tbl-cap: "Percentiles of the final balance distribution under the 100% strategy."

styled_percentiles
```

**Reading the picture.**  
The histogram and tables tell the same story: the distribution is extremely skewed. A few lucky lifetimes end with very large balances, which pull the mean to the right. But the median and the lower percentiles sit well below $30,000. For most players, the 100% strategy does not lead to a happy retirement.

## Probability that the balance exceeds $30,000 at age 75

Now I zoom in on a single quantity: starting at age 20, what fraction of lifetimes end at or above $30,000 by age 75 when we always bet 100%?
...
In words: only a minority of lifetimes finish above the starting $30,000, even though a single flip has positive expected value. For most players, the long run is unforgiving when the full account is at risk each year.

## Modified strategy – bet 25% of the balance each year

Next, I test a more cautious rule: each year only **25%** of the account is exposed to the flip, and 75% sits on the sidelines for that year.
...
# Scale guides to sit nicely under the histograms
ylim = ax.get_ylim()[1]
scale_full = ylim / max(log_pdf_full.max(), 1e-6)
scale_frac = ylim / max(log_pdf_frac.max(), 1e-6)
...
**Take‑away from the comparison.**  
The 100% strategy has a higher mean because of a handful of massive wins, but its median is much lower and its chance of finishing above $30,000 is small. The 25% strategy trims the wildest jackpots, but it raises the median and pushes a much larger share of lifetimes into the “comfortable” zone above the starting balance.
...
For this coin‑flip game (50% chance of +50%, 50% chance of −40%), the Kelly calculation produces an optimal stake fraction close to **25%** of the account per round.
:::

Seen through that lens:

- The **100% strategy** is far above Kelly. It offers a few brilliant endings and many retirements that finish below the original $30,000 after 55 years of flips.  
...
For a long investing life from **age 20 to 75**, the fractional strategy looks like a more realistic way to enjoy the positive expected value of the game without letting a long tail of bad luck completely ruin the outcome.
