...
growth_head_full = 1 + up
growth_tail_full = 1 + down

fraction = 0.25
growth_head_frac = 0.75 + 1.5 * fraction
growth_tail_frac = 0.75 + 0.6 * fraction
...
## 2) One simulated path from age 30 to 75 (100% strategy)

```{python}
#| label: single-path
#| fig-cap: "One simulated path from age 30 to age 75 when 100% of the account is at risk each year."
#| fig-width: 8
#| fig-height: 4.5
#| out-width: "100%"
...
## 3) 100 simulations – distribution for the 100% strategy
...
## 4) Probability that the balance exceeds $30,000 at age 75
...
prob_df = pd.DataFrame({
    "Metric": [
        ...
        "Simulations with final balance ≥ $30,000",
        "Probability (decimal)",
...
This table reports the estimate of \(P(\text{final balance} \ge 30{,}000)\) directly from the simulations, without repeating the number again in the surrounding text.

## 5) Modified strategy – bet 25% of the balance each year
